...
type NodeChanges<T extends ts.Node> = {
  [K in keyof T]?: T[K] | ts.Node | ts.NodeArray<ts.Node>
};
...
export function walkExpression(
  ...
  function update<T extends ts.Node>(
    changes: NodeChanges<T>,
    original: T,
    updated: T
  ): T {
  ...
  function loop(node: ts.Expression, scope: ts.Identifier[]): ts.Expression {
    if (ts.isPropertyAccessExpression(node)) {
      ...
      return visit(
        update(
          { expression },
          node,
          ts.createPropertyAccess(expression, node.name)
        ),
        scope
      );
    }
    ...
    if (ts.isElementAccessExpression(node)) {
      ...
      return visit(
        update(
          ...
          node,
          ts.createElementAccess(expression, argumentExpression)
        ),
        scope
      );
    }
    ...
    if (ts.isPrefixUnaryExpression(node)) {
      ...
      return visit(
        update({ operand }, node, ts.createPrefix(node.operator, operand)),
        scope
      );
    }
    ...
    if (ts.isPostfixUnaryExpression(node)) {
      const operand = loop(node.operand, scope);
      return visit(
        update({ operand }, node, ts.createPostfix(operand, node.operator)),
        scope
      );
    }
    ...
    if (node.kind === ts.SyntaxKind.TypeOfExpression) {
      ...
      return visit(
        update({ expression }, n, ts.createTypeOf(expression)),
        scope
      );
    }
    ...
    if (ts.isDeleteExpression(node)) {
      const expression = loop(node.expression, scope);
      return visit(
        update({ expression }, node, ts.createDelete(expression)),
        scope
      );
    }
    ...
    if (ts.isVoidExpression(node)) {
      const expression = loop(node.expression, scope);
      return visit(
        update({ expression }, node, ts.createVoid(expression)),
        scope
      );
    }
    ...
    if (ts.isBinaryExpression(node)) {
      ...
      return visit(
        update(
          { left, right },
          node,
          ts.createBinary(left, node.operatorToken, right)
        ),
        scope
      );
    }
    ...
    if (ts.isCallExpression(node)) {
      ...
      return visit(
        update(
          ...
          node,
          ts.createCall(expression, node.typeArguments, args)
        ),
        scope
      );
    }
    ...
    if (ts.isParenthesizedExpression(node)) {
      const expression = loop(node.expression, scope);
      return visit(
        update({ expression }, node, ts.createParen(expression)),
        scope
      );
    }
    ...
    if (ts.isObjectLiteralExpression(node)) {
      ...
      return visit(
        update({ properties }, node, ts.createObjectLiteral(properties)),
        scope
      );
    }
    ...
    if (ts.isArrayLiteralExpression(node)) {
      ...
      return visit(
        update({ elements }, node, ts.createArrayLiteral(elements)),
        scope
      );
    }
    ...
    if (ts.isSpreadElement(node)) {
      const expression = loop(node.expression, scope);
      return visit(
        update({ expression }, node, ts.createSpread(expression)),
        scope
      );
    }
    ...
    if (ts.isTemplateExpression(node)) {
      const templateSpans = mapNodeArray(node.templateSpans, span => {
        ...
        return update(
          { expression },
          span,
          ts.createTemplateSpan(expression, span.literal)
        );
      });
      return visit(
        update(
          { templateSpans },
          node,
          ts.createTemplateExpression(node.head, templateSpans)
        ),
        scope
      );
    }
    ...
    if (ts.isNewExpression(node)) {
      const expression = loop(node.expression, scope);
      const args =
        node.arguments && mapNodeArray(node.arguments, arg => loop(arg, scope));
      return update(
        {
          expression,
          arguments: args
  ...
  function walkObjectLiteralElementLike(
    ...
    if (ts.isPropertyAssignment(node)) {
      ...
      if (ts.isComputedPropertyName(node.name)) {
        ...
        name = update(
          { expression },
          node.name,
          ts.createComputedPropertyName(expression)
        );
      } else {
        name = node.name;
      }
    ...
    if (ts.isSpreadAssignment(node)) {
      ...
      return update(
        { expression },
        node,
        ts.createSpreadAssignment(expression)
      );
    }
...
function flatMap<T extends ts.Node, R>(
