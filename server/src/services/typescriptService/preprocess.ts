...
export function createUpdater(tsModule: T_TypeScript) {
  ...
  function modifySourceFile(
    ...
    if (isMpxFile(fileName) && !isTSLike(scriptKind)) {
  ...
  function recreateVueTempalteSourceFile(
    ...
    try {
      expressions = getTemplateTransformFunctions(tsModule).transformTemplate(
        program,
        templateCode
      );
    ...
    const sourceMapNodes = generateSourceMap(
      tsModule,
      sourceFile,
      newSourceFile
    );
    ...
    templateSourceMap[
      templateFsPath.slice(0, -".template".length)
    ] = sourceMapNodes;
  ...
  function createLanguageServiceSourceFile(
    ...
    if (isVirtualVueTemplateFile(fileName)) {
      sourceFile = recreateVueTempalteSourceFile(
        fileName,
        sourceFile,
        scriptSnapshot
      );
      ...
      tsModule.createImportDeclaration(
        ...
        setZeroPos(
          tsModule.createImportClause(
            tsModule.createIdentifier("__vueEditorBridge"),
            undefined as any
          )
        ),
...
export function injectVueTemplate(
  ...
  if (scriptSrc) {
    ...
    componentFilePath =
      "./" + path.basename(sourceFile.fileName.slice(0, -".template".length));
  }
  ...
  const helperImport = tsModule.createImportDeclaration(
    ...
    tsModule.createImportClause(
      ...
      tsModule.createNamedImports([
        tsModule.createImportSpecifier(
          undefined,
          tsModule.createIdentifier(renderHelperName)
        ),
        tsModule.createImportSpecifier(
          undefined,
        ...
        tsModule.createImportSpecifier(
          undefined,
          tsModule.createIdentifier(iterationHelperName)
        )
      ])
    ),
