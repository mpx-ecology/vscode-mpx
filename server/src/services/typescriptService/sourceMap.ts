    ...
    validNodeChildren.forEach((vc, i) => {
      ...
      if (
        ...
        const isThisInjected =
          tsModule.isPropertyAccessExpression(vc) &&
          vc.expression.kind === tsModule.SyntaxKind.ThisKeyword;
        updateOffsetMapping(
          sourceMapNode,
          isThisInjected,
          !canIncludeTrivia(tsModule, vc)
        );
 ...
 */
function foldSourceMapNodes(
  nodes: TemplateSourceMapNode[]
): TemplateSourceMapNode[] {
 ...
 */
function mapFromOffsetToOffset(
  document: TextDocument,
  offset: number,
  sourceMap: TemplateSourceMap
): number {
  ...
  for (const sourceMapNode of sourceMap[filePath]) {
    if (
      offset >= sourceMapNode.from.start &&
      offset <= sourceMapNode.from.end
    ) {
...
export function mapToRange(
  ...
  for (const sourceMapNode of sourceMap[filePath]) {
    if (
      from.start >= sourceMapNode.from.start &&
      from.start + from.length <= sourceMapNode.from.end
    ) {
 ...
 */
export function mapBackRange(
  fromDocumnet: TextDocument,
  to: ts.TextSpan,
  sourceMap: TemplateSourceMap
): Range {
  ...
  for (const sourceMapNode of sourceMap[filePath]) {
    if (
      to.start >= sourceMapNode.to.start &&
      to.start + to.length <= sourceMapNode.to.end
    ) {
...
function updateOffsetMapping(
  node: TemplateSourceMapNode,
  isThisInjected: boolean,
  fillIntermediate: boolean
) {
  const from = [...Array(node.from.end - node.from.start + 1).keys()];
  const to: (number | undefined)[] = [
    ...Array(node.to.end - node.to.start + 1).keys()
  ];
...
export function printSourceMap(
  ...
  for (const fileName in sourceMap) {
    ...
    sourceMap[fileName].forEach(node => {
      ...
      console.log(
        `[${node.from.start}, ${node.from.end}, ${sf}] => [${node.to.start}, ${
          node.to.end
        }, ${st}]`
      );
    });
...
export function stringifySourceMapNodes(
  ...
  sourceMapNodes.forEach(node => {
    ...
    result += `[${node.from.start}, ${node.from.end}, ${sf}] => [${
      node.to.start
    }, ${node.to.end}, ${st}]\n`;
  });
