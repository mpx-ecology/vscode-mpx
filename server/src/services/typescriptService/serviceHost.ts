...
import { isVirtualVueTemplateFile, isMpxFile } from './util';
...
export function getServiceHost(
  ...
  function updateCurrentVueTextDocument(doc: TextDocument) {
    ...
    if (
      ...
      if (
        localLastDoc &&
        currentScriptDoc.languageId !== localLastDoc.languageId
      ) {
      ...
      resolveModuleNames(
        ...
        const result: (ts.ResolvedModule | undefined)[] = moduleNames.map(
          name => {
            ...
            const cachedResolvedModule = moduleResolutionCache.getCache(
              name,
              containingFile
            );
            ...
            if (path.isAbsolute(name) || !isMpxFile(name)) {
              ...
              if (tsResolvedModule) {
                moduleResolutionCache.setCache(
                  name,
                  containingFile,
                  tsResolvedModule
                );
  ...
  const templateLanguageService = tsModule.createLanguageService(
    templateHost,
    registry
  );
...
function getScriptKind(tsModule: T_TypeScript, langId: string): ts.ScriptKind {
  return langId === 'typescript'
    ...
    : langId === 'json'
    ? tsModule.ScriptKind.JSON
    : tsModule.ScriptKind.JS;
}

function inferIsUsingOldVueVersion(
  tsModule: T_TypeScript,
  workspacePath: string
): boolean {
  ...
  try {
    const packageJSON =
      packageJSONPath && JSON.parse(tsModule.sys.readFile(packageJSONPath)!);
    ...
    const nodeModulesVuePackagePath = tsModule.findConfigFile(
      path.resolve(workspacePath, 'node_modules/@mpxjs'),
      tsModule.sys.fileExists,
      'package.json'
    );
    const nodeModulesVuePackageJSON =
      nodeModulesVuePackagePath &&
      JSON.parse(tsModule.sys.readFile(nodeModulesVuePackagePath)!);
    const nodeModulesVueVersion = parseFloat(
      nodeModulesVuePackageJSON.version.match(/\d+\.\d+/)[0]
    );
