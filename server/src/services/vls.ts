...
export class VLS {
  ...
  private setupConfigListeners() {
    this.lspConnection.onDidChangeConfiguration(
      async ({ settings }: DidChangeConfigurationParams) => {
  ...
  private async setupDynamicFormatters(settings: any) {
    if (settings.mpx.format.enable === true) {
  ...
  configure(config: any): void {
    const mpxValidationOptions = config.mpx.validation;
    this.validation["vue-html"] = mpxValidationOptions.template;
    this.validation.css = mpxValidationOptions.style;
    this.validation.postcss = mpxValidationOptions.style;
    this.validation.scss = mpxValidationOptions.style;
  ...
  onSignatureHelp({
    textDocument,
    position
  }: TextDocumentPositionParams): SignatureHelp | null {
    const doc = this.documentService.getDocument(textDocument.uri)!;
    const mode = this.languageModes.getModeAtPosition(doc, position);
