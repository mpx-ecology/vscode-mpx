...
export class HTMLMode implements LanguageMode {
  ...
  configure(c: any) {
    this.tagProviderSettings = _.assign(
      this.tagProviderSettings,
      c.html.suggest
    );
    this.enabledTagProviders = getEnabledTagProviders(this.tagProviderSettings);
    this.config = c;
  ...
  doComplete(document: TextDocument, position: Position) {
    ...
    const info = this.vueInfoService
      ? this.vueInfoService.getInfo(document)
      : undefined;
  ...
  format(
    ...
    return htmlFormat(document, range, this.config.vetur
      .format as VLSFormatConfig);
  }
