...
type PrettyHtmlConfig = IPrettyHtml extends (
  input: string,
  options: infer R
) => any
  ? NonNullable<R>
...
export function htmlFormat(
  ...
  const wrappedHtml = beautifiedHtml.substring(
    TEMPLATE_HEAD.length,
    beautifiedHtml.length - TEMPLATE_TAIL.length
  );
  return [
    {
      range,
...
function formatWithPrettyHtml(
  ...
  const result = prettyhtml(
    input,
    getPrettyHtmlOptions(prettierrcOptions, vlsFormatConfig)
  );
  return result.contents.trim();
}

function formatWithJsBeautify(
  input: string,
  vlsFormatConfig: VLSFormatConfig
): string {
...
function getPrettyHtmlOptions(
  prettierrcOptions: Partial<PrettierConfig> | null,
  vlsFormatConfig: VLSFormatConfig
) {
  const fromVls = {
