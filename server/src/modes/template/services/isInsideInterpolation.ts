...
export function isInsideInterpolation(
  ...
  if (
    ...
    relativePos >= "{{".length &&
    relativePos <= nodeText.length - "}}".length
  ) {
    return true;
  }

  // Case v-, : or @ directives
  const templateBody = parse(PARSER_PRE + nodeText + PARSER_POS, {})
    .templateBody;
  if (!templateBody) {
    return false;
  }
  ...
  function isInsideRange(node: { range: number[] }) {
    ...
    return (
      start - PARSER_PRE.length < relativePos &&
      end - PARSER_PRE.length > relativePos
    );
  }
}
