...
export class VueInterpolationMode implements LanguageMode {
  ...
  doValidation(document: TextDocument): Diagnostic[] {
    ...
    const {
      templateService,
      templateSourceMap
    } = this.serviceHost.updateCurrentVirtualVueTextDocument(templateDoc);
    ...
    const rawTemplateDiagnostics = templateService.getSemanticDiagnostics(
      templateFileFsPath
    );
    const templateDiagnosticFilter = createTemplateDiagnosticFilter(
      this.tsModule
    );
  ...
  doHover(
    ...
    const templateFileFsPath = getFileFsPath(templateDoc.uri);
    const mappedPosition = mapFromPositionToOffset(
      templateDoc,
      position,
      templateSourceMap
    );

    const info = templateService.getQuickInfoAtPosition(
      templateFileFsPath,
      mappedPosition
    );
    if (info) {
  ...
  findDefinition(document: TextDocument, position: Position): Location[] {
    ...
    const definitions = templateService.getDefinitionAtPosition(
      templateFileFsPath,
      mappedPosition
    );
    if (!definitions) {
    ...
    definitions.forEach(r => {
      const definitionTargetDoc =
        r.fileName === templateFileFsPath
          ? document
          : getSourceDoc(r.fileName, program);
  ...
  findReferences(document: TextDocument, position: Position): Location[] {
    ...
    const references = templateService.getReferencesAtPosition(
      templateFileFsPath,
      mappedPosition
    );
    if (!references) {
