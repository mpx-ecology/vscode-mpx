...
export function isEmptyElement(e: string | undefined): boolean {
  return (
    !!e &&
    binarySearch(EMPTY_ELEMENTS, e.toLowerCase(), (s1: string, s2: string) =>
      s1.localeCompare(s2)
    ) >= 0
  );
}
...
export const HTML_TAGS: ITagSet = {
  // The root element
  html: genTag("The html element represents the root of an HTML document.", [
    "manifest"
  ]),
  // Document metadata
  head: genTag(
  ...
  del: genTag("The del element represents a removal from the document.", [
    "cite",
    "datetime"
  ]),
  // Embedded content
  ...
  td: genTag("The td element represents a data cell in a table.", [
    "colspan",
    "rowspan",
    "headers"
  ]),
...
export function getHTML5TagProvider(): IHTMLTagProvider {
  ...
  return {
    getId: () => "html5",
    collectTags: (
      collector: (tag: string, label: string | MarkupContent) => void
