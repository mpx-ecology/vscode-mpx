...
export function getNuxtTagProvider(workspacePath: string) {
  if (
    fs.existsSync(
      path.resolve(workspacePath, NUXT_VUE_APP_PATH, "package.json")
    )
  ) {
    const { nuxtTags, nuxtAttributes } = getNuxtTagsAndAttributes(
      NUXT_VUE_APP_PATH
    );
  ...
  if (
    fs.existsSync(
      path.resolve(workspacePath, NUXT_EDGE_VUE_APP_PATH, "package.json")
    )
  ) {
    const { nuxtTags, nuxtAttributes } = getNuxtTagsAndAttributes(
      NUXT_EDGE_VUE_APP_PATH
    );
    return getExternalTagProvider("nuxt", nuxtTags, nuxtAttributes);
  }
}
...
function getNuxtTagsAndAttributes(nuxtVueAppPath: string) {
  ...
  if (nuxtVer < "2.4.0") {
    ...
    const nuxtTags = require(path.resolve(
      nuxtVueAppPath,
      "vetur/nuxt-tags.json"
    ));
    const nuxtAttributes = require(path.resolve(
      nuxtVueAppPath,
      "vetur/nuxt-attributes.json"
    ));
