...
export class VueHTMLMode implements LanguageMode {
  ...
  queryVirtualFileInfo(fileName: string, currFileText: string) {
    return this.vueInterpolationMode.queryVirtualFileInfo(
      fileName,
      currFileText
    );
  }
  doValidation(document: TextDocument) {
    return this.htmlMode
      ...
      .concat(this.vueInterpolationMode.doValidation(document));
  }
  doComplete(document: TextDocument, position: Position) {
    return this.htmlMode.doComplete(document, position);
  }
  doHover(document: TextDocument, position: Position): Hover {
    const interpolationHover = this.vueInterpolationMode.doHover(
      document,
      position
    );
    return interpolationHover.contents.length !== 0
      ? interpolationHover
      : this.htmlMode.doHover(document, position);
  }
  ...
  findDefinition(document: TextDocument, position: Position) {
    const interpolationDefinition = this.vueInterpolationMode.findDefinition(
      document,
      position
    );
