...
import {
  getLastChild,
  buildDocumentation,
  getObjectLiteralExprFromExportExpr
} from "./componentInfo";
...
export function getChildComponents(
  ...
  const componentsSymbol = checker.getPropertyOfType(
    defaultExportType,
    "components"
  );
  ...
  if (
    componentsDeclaration.kind === tsModule.SyntaxKind.ObjectLiteralExpression
  ) {
    const componentsType = checker.getTypeOfSymbolAtLocation(
      componentsSymbol,
    ...
    checker.getPropertiesOfType(componentsType).forEach(s => {
      ...
      if (s.valueDeclaration.kind === tsModule.SyntaxKind.PropertyAssignment) {
        objectLiteralSymbol =
          checker.getSymbolAtLocation(
            (s.valueDeclaration as ts.PropertyAssignment).initializer
          ) || s;
      } else if (
        s.valueDeclaration.kind ===
        tsModule.SyntaxKind.ShorthandPropertyAssignment
      ) {
        objectLiteralSymbol =
          checker.getShorthandAssignmentValueSymbol(s.valueDeclaration) || s;
      }
      ...
      if (objectLiteralSymbol.flags & tsModule.SymbolFlags.Alias) {
        const definitionObjectLiteralSymbol = checker.getAliasedSymbol(
          objectLiteralSymbol
        );
        if (definitionObjectLiteralSymbol.valueDeclaration) {
          const defaultExportExpr = getLastChild(
            definitionObjectLiteralSymbol.valueDeclaration
          );
          ...
          result.push({
            ...
            definition: {
              path: definitionObjectLiteralSymbol.valueDeclaration.getSourceFile()
                .fileName,
              start: defaultExportExpr.getStart(undefined, true),
              end: defaultExportExpr.getEnd()
            },
            defaultExportExpr: getObjectLiteralExprFromExportExpr(
              tsModule,
              defaultExportExpr
            )
          });
