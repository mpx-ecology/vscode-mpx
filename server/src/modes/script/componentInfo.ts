  ...
  if (internalChildComponents) {
    ...
    internalChildComponents.forEach(c => {
      childComponents.push({
        ...
        info: c.defaultExportExpr
          ? analyzeDefaultExportExpr(tsModule, c.defaultExportExpr, checker)
          : undefined
      });
    });
...
function getDefaultExportObjectLiteralExpr(
  ...
  const exportStmts = sourceFile.statements.filter(
    st => st.kind === tsModule.SyntaxKind.ExportAssignment
  );
...
function getData(
  ...
  const dataType = checker.getTypeOfSymbolAtLocation(
    dataSymbol,
    dataSymbol.valueDeclaration
  );
  const dataSignatures = dataType.getCallSignatures();
  if (dataSignatures.length === 0) {
    return undefined;
  }
  const dataReturnTypeProperties = checker.getReturnTypeOfSignature(
    dataSignatures[0]
  );
...
function getComputed(
  ...
  if (
    computedDeclaration.kind === tsModule.SyntaxKind.ObjectLiteralExpression
  ) {
    const computedType = checker.getTypeOfSymbolAtLocation(
      computedSymbol,
      computedDeclaration
    );
...
function getMethods(
  ...
  if (methodsDeclaration.kind === tsModule.SyntaxKind.ObjectLiteralExpression) {
    const methodsType = checker.getTypeOfSymbolAtLocation(
      methodsSymbol,
      methodsDeclaration
    );
...
export function getObjectLiteralExprFromExportExpr(
  ...
  switch (exportExpr.kind) {
    case tsModule.SyntaxKind.CallExpression:
      // mpx.extend or synthetic __vueEditorBridge
