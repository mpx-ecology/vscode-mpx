...
export async function getJsonscriptMode(
  ...
  const firstScriptRegion = getLanguageModelCache(10, 60, document => {
    const vueDocument = documentRegions.refreshAndGet(document);
    const scriptRegions = vueDocument.getLanguageRangesOfType("jsonscript");
  ...
  return {
    ...
    doValidation(doc: TextDocument): Diagnostic[] {
      const { jsonscriptDoc, service } = updateCurrentVueTextDocument(doc);
      ...
      return rawScriptDiagnostics
        ...
        .map(diag => {
          ...
          return <Diagnostic>{
            range: convertRange(jsonscriptDoc, diag as ts.TextSpan),
    ...
    doComplete(doc: TextDocument, position: Position): CompletionList {
      ...
      const fileFsPath = getFileFsPath(doc.uri);
      const offset = jsonscriptDoc.offsetAt(position);
      ...
      return {
        ...
        items: entries.map((entry, index) => {
          const range =
            ...
            convertRange(jsonscriptDoc, entry.replacementSpan);
          ...
          return {
            ...
            data: {
              ...
              languageId: jsonscriptDoc.languageId,
              uri: doc.uri,
              offset,
              source: entry.source
            }
      ...
      function calculateLabelAndDetailTextForPathImport(
        ...
        if (entry.kind === ts.ScriptElementKind.scriptElement) {
          if (entry.kindModifiers) {
            ...
            if (entry.name.endsWith(".mpx")) {
              return {
                label: entry.name.slice(0, -".mpx".length),
                detail: entry.name
              };
            }
          }
    ...
    doResolve(doc: TextDocument, item: CompletionItem): CompletionItem {
      ...
      if (
        ...
        if (details.codeActions && config.mpx.completion.autoImport) {
    ...
    doHover(doc: TextDocument, position: Position): Hover {
      ...
      const info = service.getQuickInfoAtPosition(
        fileFsPath,
        jsonscriptDoc.offsetAt(position)
      );
      if (info) {
        ...
        return {
          range: convertRange(jsonscriptDoc, info.textSpan),
    ...
    findDocumentHighlight(
      ...
      if (occurrences) {
        return occurrences.map(entry => {
          return {
            range: convertRange(jsonscriptDoc, entry.textSpan),
      ...
      const collectSymbols = (
        ...
        if (item.kind !== "script" && !existing[sig]) {
          const symbol: SymbolInformation = {
            ...
            location: {
              uri: doc.uri,
              range: convertRange(jsonscriptDoc, item.spans[0])
            },
    ...
    getCodeActions(doc, range, _formatParams, context) {
      const { jsonscriptDoc, service } = updateCurrentVueTextDocument(doc);
      const fileName = getFileFsPath(jsonscriptDoc.uri);
      const start = jsonscriptDoc.offsetAt(range.start);
      const end = jsonscriptDoc.offsetAt(range.end);
    ...
    format(
      ...
      const defaultFormatter =
        jsonscriptDoc.languageId === "jsonscript"
      ...
      const parser =
        jsonscriptDoc.languageId === "jsonscript" ? "babylon" : "typescript";
      ...
      if (
        ...
        const filePath = getFileFsPath(jsonscriptDoc.uri);
        let doFormat;
        ...
        for (const edit of edits) {
          if (
            ...
            result.push({
              range: convertRange(jsonscriptDoc, edit.span),
...
function collectRefactoringCommands(
  ...
  for (const action of actions) {
    result.push({
      command: "mpx.chooseTypeScriptRefactoring",
...
function getFormatCodeSettings(config: any): ts.FormatCodeSettings {
  return {
    tabSize: config.mpx.format.options.tabSize,
    indentSize: config.mpx.format.options.tabSize,
    convertTabsToSpaces: !config.mpx.format.options.useTabs
  };
}
