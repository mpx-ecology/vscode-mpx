...
describe("Should do documentLink", () => {
  ...
  it("shows all documentLinks for Basic.vue", async () => {
    await testLink(docUri, [
      {
        target: vscode.Uri.parse("https://vuejs.org/images/logo.png"),
        range: sameLineRange(2, 14, 47)
      },
      {
        target: getDocUri("client/documentLink/foo"),
        range: sameLineRange(3, 13, 18)
      },
      {
        target: getDocUri("client/documentLink/foo.js"),
        range: sameLineRange(7, 13, 21)
      }
    ]);
  });
});
...
async function testLink(
  ...
  function isEqualLink(h1: vscode.DocumentLink, h2: vscode.DocumentLink) {
    return (
      h1.target!.fsPath === h2.target!.fsPath && h1.range.isEqual(h2.range)
    );
  }
}
