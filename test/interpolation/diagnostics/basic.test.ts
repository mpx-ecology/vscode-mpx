      ...
      diagnostics: [
        {
          ...
          severity: vscode.DiagnosticSeverity.Error,
          message:
            "Property 'b' is protected and only accessible within class 'Child' and its subclasses"
        },
        {
          range: sameLineRange(10, 16, 17),
          severity: vscode.DiagnosticSeverity.Error,
          message:
            "Property 'c' is private and only accessible within class 'Child'"
        }
      ]
    },
    {
  ...
  tests.forEach(t => {
    it(`Shows template diagnostics for ${t.file}`, async () => {
      ...
      await testDiagnostics(
        docUri,
        t.diagnostics,
        !!t.skipSameDiagnosticCountAssert
      );
    });
  });
