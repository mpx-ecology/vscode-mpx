    ...
    if (typeof ei === "string") {
      ...
      if (ei.documentation) {
        if (typeof match.documentation === "string") {
          assert.equal(
            normalizeNewline(match.documentation),
            normalizeNewline(ei.documentation as string)
          );
        } else {
          if (
            ei.documentation &&
      ...
      if (ei.documentationStart) {
        if (typeof match.documentation === "string") {
          ...
          assert.ok(
            (match.documentation as vscode.MarkdownString).value.startsWith(
...
export async function testNoSuchCompletion(
  ...
  notExpectedItems.forEach(ei => {
    if (typeof ei === "string") {
      ...
      assert.ok(
        !match,
        `Shouldn't find perfect match for ${JSON.stringify(ei, null, 2)}`
      );
    }
  });
}
