...
function runTests(testWorkspaceRelativePath: string): Promise<number> {
  return new Promise((resolve, reject) => {
    ...
    const extTestPath = path.resolve(
      EXT_ROOT,
      "dist",
      testWorkspaceRelativePath
    );
...
async function runAllTests() {
  const testDirs = fs
    .readdirSync(path.resolve(EXT_ROOT, "./test"))
    .filter(p => !p.includes("."));
  ...
  if (targetDir && testDirs.indexOf(targetDir) !== -1) {
    try {
      installMissingDependencies(
        path.resolve(path.resolve(EXT_ROOT, `./test/${targetDir}/fixture`))
      );
    ...
    for (const dir of testDirs) {
      try {
        installMissingDependencies(
          path.resolve(path.resolve(EXT_ROOT, `./test/${dir}/fixture`))
        );
