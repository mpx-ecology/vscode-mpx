...
describe("Should do documentHighlight", () => {
    ...
    await testHighlight(docUri, position(2, 5), [
      {
        ...
        range: sameLineRange(2, 5, 8)
      },
      {
        kind: vscode.DocumentHighlightKind.Read,
        range: sameLineRange(2, 20, 23)
      }
    ]);
  });
  ...
  it("shows highlights for Item", async () => {
    await testHighlight(docUri, position(20, 16), [
      {
        kind: vscode.DocumentHighlightKind.Write,
        range: sameLineRange(17, 7, 11)
      },
      {
        kind: vscode.DocumentHighlightKind.Write,
        range: sameLineRange(20, 16, 20)
      }
    ]);
  });
});
...
async function testHighlight(
  ...
  function isEqualHighlight(
    h1: vscode.DocumentHighlight,
    h2: vscode.DocumentHighlight
  ) {
