...
describe("Should do codeAction", () => {
  ...
  it("finds codeAction for unused import", async () => {
    const codeActions = [
      {
        title: `Remove declaration for: 'lodash'`,
  ...
  it("finds codeAction for unused variables", async () => {
    const codeActions = [
      {
        title: `Remove declaration for: 'foo'`,
        command: "vetur.applyWorkspaceEdits"
      }
    ];
...
async function testCodeAction(
  ...
  expectedActions.forEach(eAction => {
    assert.ok(
      result.some(rAction => {
        return (
          rAction.title === eAction.title && rAction.command === eAction.command
        );
      }),
